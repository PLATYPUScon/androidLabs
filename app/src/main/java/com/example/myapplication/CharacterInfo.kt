...
class extend_char_info : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val item = intent.getSerializableExtra("item") as Character
        binding.apply {
            Picasso.get().load(item.imageUrl).into(iconExtend)
            charNameExtend.text = item.name

            extendedContent1.text = item.films.toString()
            extendedContent2.text = item.shortFilms.toString()
            extendedContent3.text = item.tvShows.toString()
            extendedContent4.text = item.videoGames.toString()
            extendedContent5.text = item.parkAttractions.toString()
            extendedContent6.text = item.allies.toString()
            extendedContent7.text = item.enemies.toString()
